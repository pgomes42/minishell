...
/*   Updated: 2025/10/01 11:48:04 by pgomes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_expand_env(t_data *data, char *argv[], int *start, int *last)
{
	...
	*last = i;
	if (expanded)
		expanded = ft_strjoin_f(expanded,
				ft_strdup(ft_getenv_value(data->list_env, key)));
	else
...
static char	*expanded_quote(t_data *data, char *argv[], int *start, int *last)
{
	...
	while (argv[0][++i] != '"')
	{
		if (argv[0][i] == '$' && (argv[0][i + 1] == '?' || ft_isalnum(argv[0][i
					+ 1])))
			argv[1] = ft_get_sub(argv[0], argv[1], j, i);
		if (argv[0][i] == '$' && argv[0][i + 1] == '?')
			argv[1] = ft_expander_in(data, argv, &i, &j);
...
char	*ft_expande_heroduc(t_data *data, char *line)
{
	...
	while (line && line[++i])
	{
				    ...
				    + 1] == '?'))
		{
			...
			if (line[i + 1] == '?')
				expande = ft_expander_in(data, (char *[]){line, expande}, &i, &j);
			else if (ft_isalnum(line[i + 1]))
				expande = ft_expand_env(data, (char *[]){line, expande}, &i,
						&j);
		}
	}
	expande = ft_get_sub(line, expande, j, i);
...
char	*ft_expander(t_data *data, char *args)
{
	...
	while (args && args[++i])
	{
		if (args[i] == '"' || args[i] == '\'' || (args[i] == '$' && args[i + 1]
			    && (ft_isalnum(args[i + 1]) || args[i + 1] == '?')))
			expanded = ft_get_sub(args, expanded, j, i);
		if (args[i] == '$' && args[i + 1] && args[i + 1] == '?')
			expanded = ft_expander_in(data, (char *[]){args, expanded}, &i, &j);
